...
![The core `PyBOP` architecture with base class interfaces. Each class provides a direct mapping to a step in the optimisation workflow. Note that 'inputs' means candidate parameter values being optimised, following `PyBaMM` conventions. \label{fig:classes}](figures/PyBOP_components.drawio.pdf){ width=85% }
