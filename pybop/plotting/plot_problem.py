...
def quick_plot(problem, parameter_values=None, show=True, **layout_kwargs):
    ...
    **layout_kwargs : optional
    ...
    xaxis_datasets = problem.time_data()
    model_outputs = problem.evaluate(parameter_values)
    target_outputs = problem.get_target()

    # Create a plot for each output
    figure_list = []
    for i in problem.signal:
        for target_output, model_output, xaxis_data in zip(
            target_outputs, model_outputs, xaxis_datasets
        ):
