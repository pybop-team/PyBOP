...
from multivariate_distributions import MultivariateGaussian
...
class EP_BOLFI(BaseOptimiser):
    ...
    MultivariateGaussian prior and an MultiFitting-built problem.
    """
    ...
    def _set_up_optimiser(self):
        ...
        model_bounds = self.problem._params.get_bounds(transformed=False)  # noqa: SLF001
        for i, name in enumerate(self.problem._params.keys()):  # noqa: SLF001
            transposed_boundaries[name] = [
                model_bounds["lower"][i],
                model_bounds["upper"][i],
            ]
        simulators = [fun for fun in self.problem._funs]  # noqa: SLF001
        experimental_datasets = [0.0] * len(simulators)
        feature_extractors = [lambda y: [y]] * len(simulators)
        self.optimiser = ep_bolfi.EP_BOLFI(
            ...
            free_parameters={
                ...
                for name, par in self.problem._params.items()  # noqa: SLF001
            },
            initial_covariance=self.problem._params.distribution.properties["cov"],  # noqa: SLF001
            ...
            fixed_parameter_order=list(enumerate(self.problem._params.keys())),  # noqa: SLF001
        )
    ...
    def _run(self):
        ...
        self.bolfi_posterior = self.optimiser.run(
            ...
            4,  # independent_mcmc_chains; generally works well
        ...
        self.log.log_update(
            ...
            x_search=x_list,
            cost=cost_list,
            iterations=self._options.ep_iterations,
            evaluations=len(
        ...
        """
        self.log.x_model = x_list
        self.log.x_search = [
            [
                par.transformation.to_search(e)[0]
                for e, par in zip(entry, self.problem._params.values(), strict=False)  # noqa: SLF001
            ]
            for entry in x_list
        ]
        self.log.cost = cost_list
        self.log.iterations = [
            i // (self._options.ep_iterations * len(feature_costs))
            for i in range(len(cost_list))
        ]
        self.log.evaluations = [i + 1 for i in range(len(cost_list))]
        self.log.x_model_best = x_best
        self.log.x_search_best = [
            [
                par.transformation.to_search(e)[0]
                for e, par in zip(entry, self.problem._params.values(), strict=False)  # noqa: SLF001
            ]
        ...
        search_mean_array = [
            ...
            for entry, par in zip(
                model_mean_array,
                self.problem._params.values(),  # noqa: SLF001
                strict=False,
            )
        ]
        ...
        posterior = copy.deepcopy(self.problem._params)  # noqa: SLF001
        ...
        return BayesianOptimisationResult(
            ...
            n_iterations={
                ...
                * len(self.problem._funs),  # noqa: SLF001
            },
            n_evaluations={
