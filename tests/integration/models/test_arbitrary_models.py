...
class DiffusionModel(pybamm.BaseModel):
    ...
    def __init__(self, name="Diffusion Model"):
        ...
        self.variables = {
            ...
            "x": self.x,
            "u": self.u,
            "u at x_0.5": pybamm.EvaluateAt(self.u, pybamm.Scalar(0.5)),
            "u at x_0.25": pybamm.EvaluateAt(self.u, pybamm.Scalar(0.25)),
        }
...
class TestArbitraryModels:
    ...
    def test_heat_equation(self):
        ...
        parameter_values.update(
            {"a": pybop.Parameter(initial_value=0.1, bounds=[0, np.inf])}
        )
    ...
    def test_system_odes_exp(self):
        ...
        parameter_values.update(
            {
                "a": pybop.Parameter(initial_value=-4, bounds=[-10, 0]),
                "d": pybop.Parameter(initial_value=-4, bounds=[-10, 0]),
            }
        )
    ...
    def test_system_odes_trig(self):
        ...
        parameter_values.update(
            {
                "a": 0,
                "b": pybop.Parameter(initial_value=-1, bounds=[-4, 4]),
                "c": pybop.Parameter(initial_value=-1, bounds=[-4, 4]),
                "d": 0,
                "u0": 0,
                "v0": 1,
            }
