...
sim_validate = pybamm.Simulation(model, experiment=experiment_validate, parameter_values=parameter_values)
...
dataset_validate = pybop.Dataset(
    {
        ...
        "Voltage [V]": V_data,
        "Bulk open-circuit voltage [V]": solution_validate["Bulk open-circuit voltage [V]"].entries,
    }
)
...
GP_options = {'Number of terms':num_of_terms,'arg_inds':[0],'Normalization min-max':{'0':(-1,4500)},
              'Constant mean':1.8e-10, 'Constant standard deviation':1e-11,'Bi mean':0,'Bi standard deviation':5e-11,'exp':False}

GP_param_neg = pybop.FoKLGP("Electrolyte diffusivity [m2.s-1]", parameter_values=parameter_values.copy(), options=GP_options, twoway=True)
...
new_parameters_constant.update({"Electrolyte diffusivity [m2.s-1]":
    pybop.Parameter(
        pybop.Gaussian(1.79e-10,2e-11,
                       truncated_at=[1e-14, 1e-7])
    )
...
simulator_constant = pybop.pybamm.Simulator(model, new_parameters_constant, protocol=dataset)
...
sim_final_train_constant = pybamm.Simulation(
    ...
    parameter_values=new_parameters_constant
)
...
sim_final_test_constant = pybamm.Simulation(
    model,
    experiment=experiment_validate,
    parameter_values=new_parameters_constant
)
...
for i in range(4):
    ...
    fig.add_trace(go.Scatter(
        x=t_eval, y=c_e_train[i * 19, :],
        mode='lines', name='Training'
    ))
    fig.add_trace(go.Scatter(
        x=t_eval_validate, y=c_e_test[i * 19, :],
        mode='lines', name='Testing'
    ))
    fig.add_trace(go.Scatter(
        x=t_eval_validate_GP_train, y=c_e_train_GP[i * 19, :],
        mode='lines', name='GP train'
    ))
    fig.add_trace(go.Scatter(
        x=t_eval_validate_GP, y=c_e_test_GP[i * 19, :],
        mode='lines', name='GP test'
    ))
    fig.update_layout(
        xaxis_title='Time [s]',
...
fig_val.add_trace(go.Scatter(
    x=t_eval_validate, y=V_data,
    mode='markers', name='Data',
    marker=dict(size=4)
))
fig_val.add_trace(go.Scatter(
    x=t_eval_validate_GP, y=V_test,
    mode='lines', name='FoKL GP',
    line=dict(color='green')
))
fig_val.add_trace(go.Scatter(
    x=t_eval_validate_constant, y=V_test_constant,
    mode='lines', name='PyBOP Constant',
    line=dict(color='red')
))
...
fig_train.add_trace(go.Scatter(
    x=t_eval, y=V_train_data,
    mode='markers', name='Data',
    marker=dict(size=4)
))
fig_train.add_trace(go.Scatter(
    x=t_eval_validate_GP_train, y=V_train,
    mode='lines', name='FoKL GP',
    line=dict(color='green')
))
fig_train.add_trace(go.Scatter(
    x=t_eval_validate_constant_train, y=V_train_constant,
    mode='lines', name='PyBOP Constant',
    line=dict(color='red')
))
